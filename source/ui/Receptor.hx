...
class Receptor extends FNFSprite {
  ...
  public var receptorOffsets:Array<Array<Null<Float>>> = [
		[0,0],
		[0,0],
		[0,0],
		[0,0]
    ...
    switch(behaviour.actsLike){
      case 'default':
        ...
        if(ang==null)
          ...
          var args = behaviour.arguments.receptors;

          

         //receptorOffsets[noteData] = [recepData.xOffset==null?0:recepData.xOffset,recepData.yOffset==null?0:recepData.yOffset];

         if(recepData.xOffset!=null){

          trace('found offset for ${dir} ${recepData.xOffset}');
          this.offset.x = Std.parseFloat(recepData.xOffset);
          trace(this.offset.x);
          this.updateHitbox();
         }
