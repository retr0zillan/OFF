...
typedef SaveStruct = {
	...
	@:optional var openedChest:Array<Int>;
	@:optional var elsenInteractions:Int;
...
class OptionUtils
{
    ...
    private static var elSave:FlxSave = new FlxSave();
...
class Options
{
	...
	public var fixHoldSegCount:Bool = false;
	...
	public var noteSkin:String = 'off';
...
class OptionCheckbox extends FlxSprite
{
	...
	public function new(state:Bool){
		super();
		this.state=state;
		frames = Paths.getSparrowAtlas("checkbox2");
		//updateHitbox();
		...
		antialiasing=true;
		*/
		animation.addByPrefix('unselected', 'CONFIRM Nocheck', 1, false);
		animation.addByPrefix('selected', 'CONFIRM Check', 1, false);
	...
	public function changeState(state:Bool){
		this.state=state;
		if(state){
			animation.play("selected",true,false);
		}else{
			animation.play("unselected",true,false);
		}
	}
	var daX:Int = 0;
	var daY:Int=0;
	override function update(elapsed:Float){
		...
		var left=FlxG.keys.pressed.J;
		var right=FlxG.keys.pressed.L;
		var up=FlxG.keys.pressed.I;
		var down=FlxG.keys.pressed.K;
		FlxG.watch.addQuick('Obj X', daX);
		FlxG.watch.addQuick('Obj Y', daY);
...
class ToggleOption extends Option
{
	...
	public override function createOptionFlxText(curSelected:Int,optionText:FlxTypedGroup<Option>):Dynamic{
		remove(textFlx);
		textFlx = new CustomFlxText(100,(60 + curSelected * 70), 0, name, 27);
		//textFlx.movementType = "list";
		textFlx.movementType = "list";
		textFlx.offsetX = 50;
		textFlx.gotoTargetPosition();
		checkbox.tracker=textFlx;

		//textFlx.gotoTargetPosition();
		textFlx.color = FlxColor.WHITE;
		add(textFlx);
    return textFlx;
  }
...
class ScrollOption extends Option
{
	...
	private var leftArrow:FlxSprite;
	private var rightArrow:FlxSprite;
	private var labelAlphabet:CustomFlxText;
	...
	override function update(elapsed:Float){
		labelAlphabet.targetY = textFlx.targetY;
		labelAlphabet.alpha = textFlx.alpha;
		leftArrow.alpha = textFlx.alpha;
		rightArrow.alpha = textFlx.alpha;
		super.update(elapsed);
		//sprTracker.x + sprTracker.width + 10
		if(PlayerSettings.player1.controls.LEFT && isSelected){
			leftArrow.animation.play("pressed");
		...
		rightArrow.x = textFlx.x+textFlx.width+10;
		leftArrow.x = textFlx.x-60;
		leftArrow.y = textFlx.y-10;
		rightArrow.y = textFlx.y-10;
	}

	public override function createOptionFlxText(curSelected:Int,optionText:FlxTypedGroup<Option>):Dynamic{
    remove(textFlx);
		remove(labelAlphabet);
		labelAlphabet = new CustomFlxText(100, (60 + curSelected * 70), 0, label, 27);
		labelAlphabet.movementType = "list";
		labelAlphabet.offsetX = 60;

		textFlx = new CustomFlxText(100, (60 + curSelected * 70), 0, name, 27);
		textFlx.movementType = "list";
		textFlx.offsetX = labelAlphabet.width + 120;
