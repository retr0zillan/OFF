...
class Character extends FlxSprite
{
	...
	public var beingUsed:Bool = true;
	...
	private var shaking:Bool = false; // Indica si el personaje está sacudiéndose
    private var shakeTimer:Float = 0; // Tiempo restante para la sacudida
    private var shakeDuration:Float = 1.0; // Duración total de la sacudida en segundos
    private var shakeIntensity:Float = 7.0; // Intensidad de la sacudida (ajusta según tu preferencia)
    private var originalPosition:FlxPoint = new FlxPoint(0,0);

	public function damm(){
		
		FlxTween.color(this, 0.1, this.color, 0xFFfa0116, {onComplete:function(_){
			FlxTween.color(this, 0.1, this.color, 0xFFffffff);
		}});
	...
	override function update(elapsed:Float)
	{
        ...
        if (shaking) {
            ...
            if (shakeTimer <= 0.0) {
                // La sacudida ha terminado, volvemos a la posición original
                shaking = false;
                x = originalPosition.x;
                y = originalPosition.y;
            } else {
                // Generamos una nueva posición sacudida
                var shakeOffsetX:Float = Math.random() * shakeIntensity * 2 - shakeIntensity;
                var shakeOffsetY:Float = Math.random() * shakeIntensity * 2 - shakeIntensity;
                x = originalPosition.x + shakeOffsetX;
                y = originalPosition.y + shakeOffsetY;
