...
class PlayState extends MusicBeatState
{
	...
	var dadWhiteList:Array<String>=['dies', 'valerie', 'japhet'];
	...
	public var purificationState:String = 'Pure';
	...
	public var healthTracker:Int= 0;
	public var prevInventory:PlayerInventory = new PlayerInventory();
	override public function create()
	{
		...
		super.create();
		prevInventory = MapTestState.myInventory.clone();
		...
		DiscordClient.changePresence('BATTLETIME: FACING ${MapTestState.rival.toUpperCase()}', iconRPC);
		#end
		try{
		...
		if(SONG.song.toLowerCase()=='hellish-hiems'){
			dad.playAnim('valerie');
		}
		dad.damm();
		...
		camRating.follow(camFollow,LOCKON);
		...
		battleStat = new BattleStatus(0,0, boyfriend.curCharacter);
		battleStat.somanyboxes.alpha = .70; 

		if(boyfriend.curCharacter == 'PlayableJudge'){
			MapTestState.prevHealthStats.x = MapTestState.curHealth;
			MapTestState.prevHealthStats.y = MapTestState.maxHealth;

			MapTestState.curHealth = 70;
			MapTestState.maxHealth = 70;
		}
		healthTracker = MapTestState.curHealth;
		...
		*/
		...
		//add(scoreTxt);
		  ...
		  var bars = new FlxSprite(0,0).loadGraphic(Paths.image('BlackBars1280', 'preload'));
          bars.cameras = [overlayCam];
          add(bars);
	...
	function doCountdown(countdownStatus:Int=3){
		...
		startTimer.start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if(!dadWhiteList.contains(dad.animation.curAnim.name))
			dad.dance();
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence('Purification in progress:${SONG.song} State: ${purificationState}', iconRPC, true, songLength);
		#end
	}
	...
	function updateAccuracy():Void
	{
		...
		if(judgeMan.judgementCounter.get("miss")>0){
			fcType='';
			purificationState = 'Impure';

		}else{
			
			 if(judgeMan.judgementCounter.get("bad")>0 || judgeMan.judgementCounter.get("shit")>0)
				{
					fcType += '(FC)';
					purificationState = 'Pure';
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				//DiscordClient.changePresence('BATTLETIME: FACING ${MapTestState.rival.toUpperCase()} STATE: PURE', iconRPC, true, songLength);

				DiscordClient.changePresence('Purification in progress:${SONG.song} State: ${purificationState}', iconRPC, true, songLength- Conductor.rawSongPos);
			}
			else
			{
				DiscordClient.changePresence('Purification in progress:${SONG.song} State: ${purificationState}', iconRPC);
			}
			#end
		}
	...
	{
		#if desktop
		if (MapTestState.curHealth > 0 && !paused)
		{
	...
	function pause(){
		...
		// 1 / 1000 chance for Gitaroo Man easter egg
			...
			openSubState(new OffPauseSubState(boyfriend.curCharacter));
	}
	...
	function updateCamFollow(){
		...
		camFollow.setPosition(stage.centerX,stage.centerY);
		...
		switch (event.name){
			...
			case 'Off events': 
				switch(args[0]){
					...
					case 'blackIn':
						overlayCam.fade(FlxColor.BLACK, 0.1, false);
					case 'blackOut':
						overlayCam.fade(FlxColor.BLACK, 0.1, true);
					/*
						overlayCam.fade(FlxColor.BLACK, 0.1, false, function(){
							dad.playAnim('japhet');
							new FlxTimer().start(0.3,function(_){
								overlayCam.fade(FlxColor.BLACK, 0.1, true, function(){
									new FlxTimer().start(0.3,function(_){
										overlayCam.fade(FlxColor.BLACK, 0.1, false, function(){
											dad.dance();
											new FlxTimer().start(0.3,function(_){
	...
	{
		FlxG.watch.addQuick('justPressedKeys', justPressedKeys);
		FlxG.watch.addQuick('pressedKeys', pressedKeys);
		FlxG.watch.addQuick('currentPlayer', boyfriend.curCharacter);
		...
		if(MapTestState.curHealth <0){
			MapTestState.curHealth=0;
		}
		...
		if ((controls.PAUSE||FlxG.keys.justPressed.ESCAPE) && startedCountdown && canPause)
		{
			pause();
		...
		if(!died){
			if (MapTestState.curHealth <= 0)
			{
				if(!currentOptions.noFail && !inCharter ){
					...
					MapTestState.myInventory = prevInventory.clone();
					...
					openSubState(new OffGameOverSubstate('battletime'));

					// FlxG.switchState(new GameOverState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));

					#if desktop
					// Game Over doesn't get his own variable because it's only used here
					DiscordClient.changePresence("Game Over", iconRPC);
					#end
				}else{
					died=true;
					combo=0;
		...
		if(boyfriend.animation.curAnim!=null){
			...
			{
				if (boyfriend.isSinging && !boyfriend.animation.curAnim.name.endsWith('miss')  && !skipIdle.contains(boyfriend.animation.curAnim.name))
				{
					boyfriend.dance();
				}
			}
			...
			if(startedCountdown){
			    ...
			    var notePos = modManager.getPath(diff, vDiff, daNote.noteData, daNote.mustPress==true?0:1);
					...
					if (!daNote.mustPress && daNote.canBeHit && !daNote.wasGoodHit)
					{
						...
						if(!stage.hasMultipleCharacters){
							if(!dadWhiteList.contains(opponent.animation.curAnim.name))
					...
					if(daNote!=null && daNote.alive){
						if(daNote.tooLate && daNote.mustPress && !daNote.isSustainNote && !daNote.causedMiss){
							...
							if(daNote.noteType == 'bat'){
								
								var seed = FlxG.random.int(0,3, alreadyDeathGhost);

									if(SONG.song.toLowerCase()== 'spectral-curveball'){
										var daGhost = opponents[seed];
										daGhost.playAnim('dies');
										daGhost.animation.finishCallback=function(_){
											daGhost.beingUsed = false;
											daGhost.kill();
										}
										alreadyDeathGhost.push(seed);
										trace('current ghost is ${seed}');
									}
								trace('you missed a bat note dude');
							}
						}
		...
		if(inCharter){
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					FlxG.save.flush();
					camHUD.zoom = 1;
					var boxAlpha:Float = .85;
					if(MapTestState.currentProgress<100){
						...
						switch(SONG.song.toLowerCase()){
							...
							camZooming = false;
							var dialogue = new OffDialogue(['-narrator-Tutorial has been completed'
							], Upper, true);
                                dialogue.box.alpha = boxAlpha;
                                dialogue.cameras = [camHUD];
                                add(dialogue);
								canPause = false;
								dialogue.finishThing = function(){
									var dialogue = new OffDialogue(['-judge-Very good.',
									'-judge-The tutorial is over now, remember to keep an eye on your battle status.',
								'-judge-It can make the difference between losing or winning a fight.'], Upper);
										dialogue.box.alpha = boxAlpha;
										dialogue.cameras = [camHUD];
										add(dialogue);
										dialogue.finishThing = function(){
							...
							case 'spectral-curveball':
								camZooming = false;

								var dialogue = new OffDialogue(['-narrator-Adversaries have been purified.',
							'-narrator-Batter is now level 2, Max HP has increased.', '-narrator-You have obtained 1 Lucky Ticket.'], Upper);
                               dialogue.box.alpha = boxAlpha;
                                dialogue.cameras = [camHUD];
                                add(dialogue);
								canPause = false;
								dialogue.finishThing = function(){
									MapTestState.playerLevel = 2;
									MapTestState.curHealth = 180;
									MapTestState.maxHealth = 180;
									MapTestState.myInventory.addItem('Lucky ticket', 1, 'Restores a moderate amount of health');
							...
							case 'iron-fist':
								...
								var dedanTalks = new OffDialogue(['-dedan-Is this... a... joke?...', '-dedan-I...', '-dedan-I...Lost?', "-batter-You've been defeated Dedan, guardian of the first zone", '-batter-This land is now pure.'], Upper);
								dedanTalks.box.alpha = boxAlpha;
								dedanTalks.cameras = [camHUD];
								add(dedanTalks);
								dedanTalks.finishThing = function(){


									new FlxTimer().start(0.7, function(_){
										FlxTween.tween(dad, {alpha:0}, 5, {onComplete:function(_){

											new FlxTimer().start(1, function(_){
												var dialogue = new OffDialogue(['-narrator-Adversaries have been purified.',
												'-narrator-Batter is now level 3, Max HP has increased.', '-narrator-You have obtained 3 Fortune Tickets.'], Upper);
													dialogue.box.alpha = boxAlpha;
													dialogue.cameras = [camHUD];
													add(dialogue);
													canPause = false;
													dialogue.finishThing = function(){
														MapTestState.playerLevel = 3;
														MapTestState.curHealth = 200;
														MapTestState.maxHealth = 200;
														MapTestState.myInventory.addItem('Fortune ticket', 3, 'Restores a large amount of health');
							...
							case 'hellish-hiems':
								camZooming = false;
									...
									var japhetTalks = new OffDialogue(['-japhet-So, I have been slain...', '-japhet-You can be proud to have accomplished your mission, Batter...', "-batter-You've been defeated Japhet, guardian of the second zone.", '-batter-This land is now pure.'], Upper);
									japhetTalks.box.alpha = boxAlpha;
									japhetTalks.cameras = [camHUD];
								add(japhetTalks);
								japhetTalks.finishThing = function(){
									...
									new FlxTimer().start(0.7, function(_){
										FlxTween.tween(dad, {alpha:0}, 5, {onComplete:function(_){
											...
											new FlxTimer().start(1, function(_){
												var dialogue = new OffDialogue(['-narrator-Adversaries have been purified.','-narrator-You have obtained 2 Lucky Tickets.'], Upper);
												dialogue.box.alpha = boxAlpha;
												dialogue.cameras = [camHUD];
												add(dialogue);
												canPause = false;
												dialogue.finishThing = function(){
													...
													MapTestState.myInventory.addItem('Lucky ticket', 2, 'Restores a moderate amount of health');
							...
							case 'a-furia-do-colosso':
								camZooming = false;

								

									var enochTalks = new OffDialogue(['-enoch-hhh...hhh...', '-enoch-So... the hero has destroyed the beast...','-enoch-But do you know what you have just done?',
									 '-enoch-This zone, deprived of its guardian, is now destined to disappear...'
									, "-batter-You've been defeated Enoch, guardian of the third zone.", '-batter-This land is now pure.'], Upper);
									enochTalks.box.alpha = boxAlpha;
									enochTalks.cameras = [camHUD];
								add(enochTalks);
								enochTalks.finishThing = function(){
									...
									new FlxTimer().start(0.7, function(_){
										FlxTween.tween(dad, {alpha:0}, 5, {onComplete:function(_){
											...
											new FlxTimer().start(1, function(_){
												var dialogue = new OffDialogue(['-narrator-Adversaries have been purified.',
											'-narrator-Batter is now level 4, Max HP has increased.', '-narrator-You have obtained 2 Chunks Of Meat.'], Upper);
												dialogue.box.alpha = boxAlpha;
												dialogue.cameras = [camHUD];
												add(dialogue);
												canPause = false;
												dialogue.finishThing = function(){
													MapTestState.playerLevel = 4;
													MapTestState.curHealth = 230;
													MapTestState.maxHealth = 230;
													MapTestState.myInventory.addItem('Chunk of meat', 2, 'Restores a considerable amount of health');
							...
							case 'melancholy-symphony':
								camZooming = false;

								

									var queenTalks = new OffDialogue(['-queen-That was very irresponsible on your part, Batter...', '-queen-I even made a big cake for the party tonight...',
									 '-queen-Do you want some coffe... my love...', "-batter-Queen of the flies, it's time to join your disciples.",
									  '-batter-It all went wrong. Time to forget about it and dream sweet dreams.', '-queen-Look... he has your eyes...',
									   '-batter-They are full of fear.'], Upper);
									queenTalks.box.alpha = boxAlpha;
									queenTalks.cameras = [camHUD];
								add(queenTalks);
								queenTalks.finishThing = function(){
									...
									new FlxTimer().start(0.7, function(_){
										FlxTween.tween(dad, {alpha:0}, 5, {onComplete:function(_){
											...
											new FlxTimer().start(1, function(_){
												var dialogue = new OffDialogue(['-narrator-Adversaries have been purified.',
												'-narrator-Batter is now level 5, Max HP has increased.', '-narrator-You have obtained 1 Fortune Ticket'], Upper);
													dialogue.box.alpha = boxAlpha;
													dialogue.cameras = [camHUD];
													add(dialogue);
													canPause = false;
													dialogue.finishThing = function(){
														MapTestState.playerLevel = 5;
														MapTestState.curHealth = 260;
														MapTestState.maxHealth = 260;
														MapTestState.myInventory.addItem('Fortune ticket', 1, 'Restores a large amount of health');
							...
							case 'hugo':
								camZooming = false;
									...
									var hugoTalks = new OffDialogue(["-hugo-I'm... scared of the dark...", '-batter-From now on, there will be no more darkness.'], Upper);
									hugoTalks.box.alpha = boxAlpha;
								   hugoTalks.cameras = [camHUD];
							   add(hugoTalks);
							   hugoTalks.finishThing = function(){
								   ...
								   new FlxTimer().start(0.7, function(_){
											...
											dad.animation.finishCallback = function(_){
												...
												dad.kill();
												new FlxTimer().start(1, function(_){
													var dialogue = new OffDialogue(['-narrator-Adversaries have been purified.',  "-narrator-You have obtained Hugo's Favourite Drumstick"], Upper);
													dialogue.box.alpha = boxAlpha;
													dialogue.cameras = [camHUD];
													add(dialogue);
													canPause = false;
													dialogue.finishThing = function(){
														MapTestState.myInventory.addItem("Meat Drumstick", 1, 'Hugo used to love this');
							...
							case 'ascendants-echo':
								camZooming = false;

								var judgeTalks = new OffDialogue(['-judge-Your demented crusade ends here.',
							'-judge-Die, Batter, with the eternal souvenir of not having conducted your sad scheme to its end',
						"-batter-It's too late, Everything is lost.", '-judge-I know, but i prefer this over your victory.'], Upper);
								judgeTalks.box.alpha = boxAlpha;
								judgeTalks.cameras = [camHUD];
								add(judgeTalks);

								judgeTalks.finishThing = function(){
									dad.playAnim('dies');
									dad.animation.finishCallback=function(_){
	
										dad.beingUsed = false;
										dad.kill();
										var dialogue = new OffDialogue(['-narrator-Batter has been eliminated.'], Upper);
										dialogue.box.alpha = boxAlpha;
										dialogue.cameras = [camHUD];
										add(dialogue);
										canPause = false;
										dialogue.finishThing = function(){
											...
											MapTestState.curHealth = Std.int(MapTestState.prevHealthStats.x);
											MapTestState.maxHealth = Std.int(MapTestState.prevHealthStats.y);
											FlxG.switchState(new MapTestState(MapTestState.lastMap));
							...
							case 'no-way-back':
								...
								var batterTalks = new OffDialogue(["-batter-It's over.", '-batter-Escaping from your purpose is impossible.'], Upper);
							batterTalks.box.alpha = boxAlpha;
							batterTalks.cameras = [camHUD];
							add(batterTalks);
	
							batterTalks.finishThing = function(){
								new FlxTimer().start(0.7, function(_){
									FlxTween.tween(dad, {alpha:0}, 5, {onComplete:function(_){
										var dialogue = new OffDialogue(['-narrator-Adversaries have been purified.'], Upper);
										dialogue.box.alpha = boxAlpha;
										dialogue.cameras = [camHUD];
										add(dialogue);
										canPause = false;
					...
					else if(MapTestState.currentProgress>=100)
						{
							switch(SONG.song.toLowerCase()){
								case 'ascendants-echo':
								...
								case 'bis-vincit':
										...
										camZooming = false;
										var zachTalks = new OffDialogue(['-zacharie-That was pretty fun!.'], Upper);
										zachTalks.box.alpha = boxAlpha;
										zachTalks.cameras = [camHUD];
											add(zachTalks);
			
											zachTalks.finishThing = function(){
												FlxG.switchState(new MapTestState(MapTestState.lastMap));
	...
	private function keyPress(event:KeyboardEvent) {
		...
		// Verificar si se presionaron todas las direcciones
		var allDirectionsPressed = true;
		for (i in 0...bindData.length) {
			if (!pressedKeys[i]) {
				allDirectionsPressed = false;
				break;
			}
		}
	
		if (allDirectionsPressed) {
			trace("player pressed all directions");
			handleMultipleInput();

		}
	}

	var alreadyDeathGhost:Array<Int>=[];
	private function handleMultipleInput(){
		for(note in playerNotes){
			if(note.canBeHit && note.alive && !note.wasGoodHit)
				{
					if(note.noteType=='bat'){
	...
	{
		//MapTestState.curHealth += judgeMan.getJudgementHealth('miss');
		#if !debug
		switch(MapTestState.rival){
			case 'Judge':
				MapTestState.curHealth -= 15;
			case 'Ghost':
				MapTestState.curHealth -= 30;
			case 'Dedan':
				MapTestState.curHealth -= FlxG.random.int(40, 90);
				case 'Japhet':
					MapTestState.curHealth -= FlxG.random.int(60, 90);
				case 'Enoch':
					MapTestState.curHealth -= FlxG.random.int(110, 130);
				case 'Queen':
					MapTestState.curHealth -= FlxG.random.int(140, 190);
				case 'Hugo':
				MapTestState.curHealth -= 1;
				case 'FinalJudge':
					MapTestState.curHealth -= FlxG.random.int(190, 200);
				case 'Batter':
					MapTestState.curHealth -= FlxG.random.int(5, 70);
				case 'Zacharie':
					MapTestState.curHealth -= FlxG.random.int(10, 20);
				case 'Cow':
					MapTestState.curHealth -= FlxG.random.int(100, 190);
		...
		FlxG.sound.play(Paths.soundRandom('hurt', 1, 3));
	...
	function hitIt(){
		boyfriend.holdTimer = 0;
			...
			if(boyfriend.animation.curAnim!=null){
				boyfriend.playAnim('attack${FlxG.random.int(1,2)}',true);
	...
	override function beatHit()
	{
		...
		switch(stage.curStage){
			case 'yellow':
				if(MapTestState.currentProgress == 15){
					switch(curBeat){
						case 4:
							inst.fadeOut(4,0, function(_){
								forcePause = true;
								camHUD.zoom = 1;
								...
								new FlxTimer().start(1.5, function(_){
									canPause = false;
									var dialoguee = CoolUtil.coolTextFile2(File.getContent(Paths.txt('dialogue/judgeTutorial')));

									var dialogue = new OffDialogue(dialoguee, Upper);
									dialogue.box.alpha = .85;
			...
			if(!stage.hasMultipleCharacters)
				{
					if(dad.animation.curAnim!=null)
						if (!dad.isSinging && !dadWhiteList.contains(dad.animation.curAnim.name))
							dad.dance();
					...
					for(opponent in opponents){
						if(opponent.beingUsed){
							if(opponent.animation.curAnim!=null)
								if (!opponent.isSinging && !dadWhiteList.contains(opponent.animation.curAnim.name))
									{
										opponent.dance();
		...
		if(boyfriend.animation.curAnim!=null)
			if (!boyfriend.isSinging && !skipIdle.contains(boyfriend.animation.curAnim.name))
				boyfriend.dance();
